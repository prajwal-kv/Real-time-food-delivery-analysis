...
with DAG(
    ...
    load_dimCustomers = S3ToRedshiftOperator(
        ...
        table='dimCustomers',
        s3_bucket='food-delivery-data-analysis-one',# S3-bucket id
        s3_key='dims/dimCustomers.csv', #S3 folder name inside that bucket
...
# First, create the schema,then running the multiple tasks in parallel to drop existing tables
...
#first create dimension tables then fact table as fact table depends on dimension tables
...
# only dimension tables are loaded from S3 in this DAG
# only after loading the dimension tables the next dag is triggered
# fact table will be loaded in the next dag using spark streaming job
